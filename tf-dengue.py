...
x=data.iloc[:, [1,2,3,4,6,7,8,9,10,12,14,15,16,17,19,20]].values #type:numpy.ndarray
x_pred=dftest_midx.iloc[:, [0,1,2,3,5,6,7,8,9,11,13,14,15,16,18,19]].values
...
imputation= Imputer(missing_values='NaN', strategy='median', axis=0)
...
pipeline = make_pipeline(imputation, scaler,pca)
...
#split the 2 PCA features into 2 arrays 
df=pd.DataFrame(pca_features)
pca1= df[0].values
pca2= df[1].values


all_x1 = pca1.reshape(-1, 1)
all_x2 = pca2.reshape(-1, 1)
all_ys = y.reshape(-1, 1)
dataset_size = len(y)
...
W1x1 = tf.matmul(x1, W1)
W2x2 = tf.matmul(x2, W2)
y = W1x1 + W2x2 + b
...
def trainWithMultiplePointsPerEpoch(steps, train_step, batch_size):
  ...
  with tf.Session() as sess:
    ...
    for i in range(steps):
      ...
      batch_all_x1 = all_x1[batch_start_idx : batch_end_idx]
      batch_all_x2 = all_x2[batch_start_idx : batch_end_idx]
      batch_ys = all_ys[batch_start_idx : batch_end_idx]

      feed = { x1:  batch_all_x1, x2: batch_all_x2 , y_: batch_ys }
